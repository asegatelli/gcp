...
table_marca_ano_mes = 'refined.tb_vendas_marca_ano_mes'
...
def agg_marca_ano_mes(df):
    df = df.select(f.col('marca'), f.col('ano_venda'),f.col('mes_venda'), f.col('qtd_venda'))
    df_marca_ano_mes = df.groupBy(f.col('marca'),f.col('ano_venda'),f.col('mes_venda'))\
	               .agg(f.sum(f.col('qtd_venda'))\
				   .alias('qtd_venda'))
				   
    return df_marca_ano_mes


def orquestrador(df, log, table): 
    df_agg = agg_marca_ano_mes(df)
    del df
    log.info(f'saving {table_marca_ano_mes}')
    try:
        save(df_agg, bucket_temp,table_marca_ano_mes )
...
def main():
    ...
    orquestrador(df_trusted,log,table_marca_ano_mes)
    
    log.info(f'finalizando ingestão {table_marca_ano_mes}')
