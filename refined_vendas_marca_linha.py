...
table_marca_linha = 'refined.tb_vendas_marca_linha'
...
def agg_marca_linha(df):
    df = df.select(f.col('marca'), f.col('linha'), f.col('qtd_venda'))
    df_marca_linha = df.groupBy(f.col('marca'),f.col('linha'))\
	               .agg(f.sum(f.col('qtd_venda'))\
				   .alias('qtd_venda'))
				   
    return df_marca_linha


def orquestrador(df, log, table): 
    df_agg = agg_marca_linha(df)
    del df
    log.info(f'saving {table_marca_linha}')
    try:
        save(df_agg, bucket_temp,table_marca_linha )
...
def main():
    ...
    orquestrador(df_trusted,log,table_marca_linha)
    
    log.info(f'finalizando ingestão {table_marca_linha}')
