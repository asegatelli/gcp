...
table_linha_ano_mes = 'refined.tb_vendas_linha_ano_mes'
...
def agg_linha_ano_mes(df):
    df = df.select(f.col('linha'), f.col('ano_venda'),f.col('mes_venda'), f.col('qtd_venda'))
    df_linha_ano_mes = df.groupBy(f.col('linha'),f.col('ano_venda'),f.col('mes_venda'))\
	               .agg(f.sum(f.col('qtd_venda'))\
				   .alias('qtd_venda'))
				   
    return df_linha_ano_mes


def orquestrador(df, log, table): 
    df_agg = agg_linha_ano_mes(df)
    del df
    log.info(f'saving {table_linha_ano_mes}')
    try:
        save(df_agg, bucket_temp,table_linha_ano_mes )
...
def main():
    ...
    orquestrador(df_trusted,log,table_linha_ano_mes)
    
    log.info(f'finalizando ingestão {table_linha_ano_mes}')

if __name__ == '__main__':
    spark = SparkSession.Builder()\
                       .appName("Refined_vendas_marca_linha")\
					   .getOrCreate() 
    main()
